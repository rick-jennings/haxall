...
# The following runs hx init with the given database name from build argument HAXALL_DB_NAME (default is "var").
# It can be changed when doing docker compose commands like so: --build-arg HAXALL_DB_NAME=example
# It is stored under an ENV variable of same name (HAXALL_DB_NAME) created during the dockerfile build context, and persists after.
if [ -d "$HAXALL_DB_NAME" ]; then
    ...
    # The directory exists, and is NOT empty.
        ...
        echo "$HAXALL_DB_NAME already exists." && echo "If you want to modify your superuser account or the HTTP port, run the 'fan hx init' command again on the same directory, like so:" && echo "fan hx init $HAXALL_DB_NAME"
    fi
